...
- `src/shared/preload-contract.js` — Node-testable copy of preload contract validators. Preload inlines this logic because sandboxed preloads cannot require local files.
...
- `scripts/lib/electron-test-harness.js` — Node-only Electron/CDP test Adapter for launch/connect/wait/evaluate/viewport/screenshot/teardown plus a higher-level browser driver Interface for renderer readiness, visible-element clicks, key input, default-character startup, intro dismissal, safe stale-lock cleanup, and captured Electron stdout/stderr artifacts. `scripts/real-startup-no-blank-command-test.js` now uses this deeper Adapter as the first migrated real Electron lifecycle proof; `scripts/electron-test-harness-contract-test.js` covers non-text key dispatch and SIGKILL teardown escalation.
...
- `src/preload.js` still inlines validators mirrored by `src/shared/preload-contract.js` because the sandbox cannot require local files. `scripts/preload-contract-drift-test.js` catches drift; a future generated checked-in validator block would improve Locality.
...
`renderer.html` loads: xterm, module-system, command-gateway, message-log, prompt-rules, tile-assets, character-options, interaction-model, public-blockers, inventory-action-service, map-presentation, shim-protocol, ui-protocol-v2, menu-metadata-adapter, inventory-snapshot-adapter, equipment-snapshot-adapter, ground-pile-snapshot-adapter, container-contents-snapshot-adapter, command-transaction-model, transfer-transaction-model, game-view-state, recording-schema, status-hud, then `renderer.js`.
