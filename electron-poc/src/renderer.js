...
const settingsStorageKey = 'nethack-electron-poc-settings-v1';
const defaultSettings = { contextualMenus: true, autoLootGold: true };
...
function mapTooltipInfoForCell(cell, x, y) {
  if (sharedModules.mapPresentation?.tooltipInfoForCell) return sharedModules.mapPresentation.tooltipInfoForCell(cell, x, y, { tileMapConfig, tileAssetsById, cells: mapCells, playerCharacter: currentPlayerCharacter() });
  const normalized = normalizeCell(cell);
  const ch = normalized.ch || ' ';
  const assetId = mappedAssetIdForCell(normalized);
  const tile = assetId ? tileAssetsById.get(assetId) : undefined;
  ...
  return { title: titleCase(title), description: details.join(' · '), tile, assetId, glyph: ch, isCorpse, isStatue };
}
...
function positionMapTooltip(cellEl) {
  ...
  mapTooltip.style.left = `${Math.round(Math.max(margin, left))}px`;
...
function showMapTooltipForCell(cellEl) {
  ...
  const nextKey = `${x},${y}:${info.assetId || info.glyph}:${info.title}`;
...
function renderMessages({ forceScroll = false } = {}) {
  ...
  if (!messageHistory.length) {
    messages.textContent = 'Waiting for shim messages…';
...
function saveSettings(nextSettings = userSettings) {
  userSettings = { ...defaultSettings, ...nextSettings };
  try { window.localStorage?.setItem(settingsStorageKey, JSON.stringify(userSettings)); } catch {}
  syncSettingsForm();
  return userSettings;
}
...
function resetGameView() {
  ...
  renderMessages();
  statusLines.textContent = 'Waiting for shim status…';
...
function mergeVisibleGroundItemsWithExistingPublicIds(items = [], coord = groundPileCoordHere(), { complete = true } = {}) {
  ...
  const adapter = sharedModules.groundPileSnapshotAdapter;
  if (adapter?.reconcileGroundPileObservation) return adapter.reconcileGroundPileObservation(existingItems, items, { complete });
...
function containerSnapshotIdentity() {
  ...
  return { publicId, displayName, ...(objectId != null ? { objectId } : {}) };
}
...
function appendMessage(text, { allowConsecutiveDuplicate = false, logPrompt = true } = {}) {
  ...
  if (result) {
    ...
    if (result.appended) maybeShowContextualPrompt(String(text || '').trim());
...
function sendTravelToCell(cellEl) {
  ...
  appendMessage(`GUI travel: map ${x},${y}.`);
  gameGrid.focus({ preventScroll: true });
}
...
function showLockedDoorPrompt(message) {
  ...
  showInteractionDialog({
    ...
    options: [
      ...
      { key: '', className: 'context-choice', label: 'Ignore', text: 'Close this suggestion without sending a command.', onClick: dismissContextualPrompt },
    ],
  });
}
...
function isContainerUnlockPrompt(query = '') {
  return /unlock/i.test(String(query || '')) && /(?:credit card|lock pick|key|tool)/i.test(String(query || '')) && /(?:box|chest|container|large box|ice box|bag|sack|it)/i.test(String(query || ''));
}
...
function maybeContinueContainerOpenAfterUnlock(message = '') {
  ...
  if (/\b(?:succeed|unlock|picked|click)\b/i.test(text) && !/fail|cannot|can't|not|unable/i.test(text)) {
    ...
    window.setTimeout(async () => {
      ...
      if (result !== false && result?.ok !== false) appendMessage('GUI opened the unlocked container through a direct NetHack snapshot.');
      pendingContainerUnlockOpen = null;
    }, 120);
...
function buildContextActions() {
  const actions = [
    ...
    { id: 'wait', label: 'Wait', command: 'key', key: '.' },
...
async function runContextAction(action) {
  ...
  if (actionDispatched && !isShopPaymentAction) appendMessage(`GUI contextual action: ${action.label}.`);
  gameGrid.focus({ preventScroll: true });
}
  ...
  for (const action of actions) {
    ...
    button.title = /^pay-shopkeeper-/.test(action.id || '')
      ...
      : (action.id === 'open-container' || (action.command === 'ext' && /^Open\b/i.test(action.label || '') && /box|chest|bag|container/i.test(action.label || '')))
        ? `${action.label}; opens the container panel through a direct NetHack snapshot.`
        : (action.command === 'direction' ? `${action.label}; direction is sent by the GUI.` : (action.command === 'ext' || action.command === 'ext-direction' ? `${action.label}; sends a # command internally and wraps follow-up prompts in GUI controls.` : action.label));
...
function renderStatusLines() {
  ...
  statusLines.textContent = fields.length ? fields.join('  |  ') : 'Waiting for shim status…';
  statsPanel.textContent = '';
  const fragment = document.createDocumentFragment();
...
function backingInventoryActionSurfaceIdentity() {
  ...
  return {
    window: currentMenu?.window ?? null,
    requestId: String(currentMenu?.requestId || currentMenu?.menuRequestId || ''),
...
function sendActionCommandFromInventorySurface(keys, options = {}) {
  ...
  const sendCommand = () => {
    if (sent) return;
    sent = true;
    const expectedRevision = options.recomputeExpectedRevisionBeforeSend ? currentActionExpectedRevision() : options.expectedRevision;
    if (options.semanticAction) sendSemanticActionCommand(command, options.semanticAction, options.item, options.route, { source: options.source || 'semantic-action', expectedRevision, target: options.target, payload: options.payload });
  ...
  const timeoutAt = performance.now() + Number(options.cancelTimeoutMs || 2500);
  const sendAfterCancel = () => {
    if (!backingInventoryActionSurfaceStillActive(backingSurface)) {
      sendCommand();
      return;
    }
    if (performance.now() >= timeoutAt) {
      const actionLabel = String(options.semanticAction?.label || options.route?.label || 'That action').replace(/…/g, '');
      setEquipmentDropFeedback(`${actionLabel} is waiting for the inventory menu to close before sending its selector.`);
      return;
    }
    window.setTimeout(sendAfterCancel, 20);
  };
  sendPlayableText('\u001b');
  window.setTimeout(sendAfterCancel, 20);
}
...
function executeInventoryAction(item, affordance) {
  ...
  if (affordance?.id === 'item.inspect' || !keys) {
    setEquipmentDropFeedback(route.ok === false ? (route.reason || 'That action is not available.') : `${cleanEquipmentText(item?.text || 'Item')}: inspect/details need structured item-details protocol; no turn spent.`);
    return;
  }
  ...
  sendActionCommandFromInventorySurface(keys, {
    ...
    recomputeExpectedRevisionBeforeSend: equipmentAction,
...
function showEquipmentSlotContextMenu(slot, anchorOrEvent) {
  ...
  menu.style.left = `${Math.min(Math.max(8, isPointerEvent ? anchorOrEvent.clientX : rect.left), Math.max(8, window.innerWidth - 340))}px`;
  menu.style.top = `${Math.min(Math.max(8, isPointerEvent ? anchorOrEvent.clientY : rect.bottom), Math.max(8, window.innerHeight - Math.min(560, menu.offsetHeight || 420)))}px`;
...
function executeGroundItemAction(item, affordance) {
  ...
  if (route === 'pickupThenInventoryAction') {
    ...
    appendMessage(`GUI ground action: picked up ${itemName}; next use ${affordance.params?.afterLabel || 'the matching inventory action'} from the visible controls.`);
  } else if (route === 'pickupGroundItem') {
    sendPlayableText(keys);
    appendMessage(`GUI ground action: pick up ${itemName}.`);
    ...
    appendMessage(`GUI ground action: ${affordance.label}.`);
...
function renderEquipmentScreenPanel(items) {
  ...
  for (const slot of slotModels) slots.appendChild(equipmentSlotElement(slot, true));
...
function focusInteractionChoice(delta = 1) {
  const choices = visibleInteractionChoices();
  if (!choices.length) return false;
  const current = document.activeElement;
  const currentIndex = choices.indexOf(current);
  const nextIndex = currentIndex < 0 ? 0 : (currentIndex + delta + choices.length) % choices.length;
...
function handleInteractionNavigationKeydown(event) {
  ...
  if (event.key === 'Escape') cancelActiveInteraction();
    ...
    const choices = visibleInteractionChoices();
    if (choices.length) choices[choices.length - 1].focus({ preventScroll: true });
  } else if (event.key === 'Enter' || event.key === ' ' || event.key === 'Spacebar') activateFocusedInteractionControl();
  return true;
}

function showInteractionDialog({ title, prompt, options = [], cancelText = 'Cancel / Esc', textEntry = false, textLabel = 'Filter or type selection', textPlaceholder = '', dialogClass = '', confirmText = '', clearText = 'Clear selection', selectAllText = 'Select visible', onConfirm, onClear, onSelectAll, feedback, panelControls = null, contextLines = [] }) {
  ...
  for (const option of options) {
    ...
    button.setAttribute('role', option.role || 'option');
  ...
  if (textEntry) {
    ...
    const [firstChoice] = visibleInteractionChoices();
    if (firstChoice) firstChoice.focus({ preventScroll: true });
    else interactionCancel.focus({ preventScroll: true });
  }
  const applyFilter = () => {
...
function groundItemsFromTextWindowLines(lines) {
  ...
  for (const [index, line] of itemLines.entries()) {
    ...
    rows.push({ selector: 0, syntheticSelector: `ground-${index}`, text, syntheticGroundItem: true, semanticKind: 'object' });
  }
  return rows;
}
...
function openDocumentWindow({ title, lines }) {
  ...
  documentWindow = { title, lines: cleanLines };
  documentFilter.value = '';
  renderDocumentWindow();
  if (!documentDialog.open) documentDialog.showModal();
  documentClose?.focus({ preventScroll: true });
}
  ...
  return Array.from(promptSelectorSet(query, choices))
    ...
    .map((key) => {
      ...
      return {
        ...
        ariaLabel: `${special.label}; NetHack selector ${hint}`,
...
function selectorFallbackOptions(query, choices) {
  ...
  return selectors.map((key) => {
    ...
    return special ? {
      ...
      text: special.text,
      className: 'letter-choice special-selector-choice gui-visible-choice',
      ariaLabel: `${special.label}; NetHack selector ${key}`,
      ...
      ariaLabel: `Item selector ${key}; name unavailable`,
...
function itemClassOptions(query, choices) {
  ...
  return keys.map((key) => {
    ...
    return {
      ...
      ariaLabel: `${label}; ${description}; NetHack selector ${key}`,
...
function choiceButtonOptions(query, choices) {
  return String(choices || '').split('').filter(Boolean).map((key, index) => {
    ...
    return {
      ...
      ariaLabel: `${visibleLabel}; NetHack selector ${hint}${danger ? '; serious confirmation' : ''}`,
...
function renderDirectionHelper(query, { promptActive = true } = {}) {
  ...
  directionHelperTitle.textContent = promptActive ? 'Direction' : 'Movement';
  ...
  for (const option of directionChoiceOptions()) {
    ...
    button.addEventListener('click', () => {
      if (option.key === '.') sendMovementCommand(option.key);
      else sendPlayableText(option.key);
      gameGrid.focus({ preventScroll: true });
    });
...
function containerTransferReadyStatusText() {
  ...
  if (leftLoaded && rightLoaded) return isGroundPickup ? 'Ground items ready.' : 'container transfer panel ready';
  ...
  if (activePrompt.kind === 'question') {
    ...
    const inventoryRows = inventoryOptions.map((item) => {
      ...
      return {
        ...
        ariaLabel: `Choose ${menuItemName(item.text)}${menuItemState(item.text) ? `, ${menuItemState(item.text)}` : ''}; NetHack selector ${key}`,
    ...
    if (directionPrompt) {
      ...
      promptPanel.textContent = 'No active prompt.';
      setMapTargetMode(null);
      setStatus('direction prompt: use movement keys or compact direction controls');
    ...
    showInteractionDialog({
      ...
      confirmText: commandRows.length ? 'Submit typed command' : 'Confirm / Enter',
    ...
    showInteractionDialog({ title: workflowPromptText('Extended command (#)'), prompt: workflowPromptText(activePrompt.query || 'Choose an extended command.'), options, textEntry: true, textLabel: 'Filter commands', textPlaceholder: 'Filter commands…' });
  }
}
...
function isPromptlessObjectSelectionMenu(menu) {
  ...
  if (!Boolean(menu?.awaitingSelection) || !selectable.length) return false;
  if (prompt && !/^Menu$/i.test(prompt)) return false;
  return selectable.every((item) => item.semanticKind === 'object' || /^[a-z$]\s+-\s+/i.test(String(item.text || '')));
}
...
function isContainerExpectedTakeOutMenu(menu) {
  return isContainerTakeOutMenu(menu) || (isPromptlessObjectSelectionMenu(menu) && (expectedContainerLoadingSide() === 'left' || currentPendingContainerTransferSelection()?.action === 'out'));
}
...
function isContainerInventoryProbeMenu(menu) {
  ...
  if (!isPromptlessObjectSelectionMenu(menu)) return false;
  const choreography = currentTransferChoreographyState({ kind: 'container' });
...
function isContainerExpectedPutInMenu(menu) {
  return isContainerPutInMenu(menu) || (isPromptlessObjectSelectionMenu(menu) && (expectedContainerLoadingSide() === 'right' || currentPendingContainerTransferSelection()?.action === 'in' || isContainerInventoryProbeMenu(menu)));
}
...
function reopenContainerActionMenuForAuto(side) {
  ...
  containerTransferState.feedback = side === 'right' ? 'Refreshing inventory candidates from public inventory state…' : 'Refreshing container contents through a direct NetHack snapshot…';
...
function transferRowsForOwnership(side) {
  return containerMenuItems(side).map((item) => ({ selector: transferItemKey(item), text: containerTransferDisplayName(item, side), objectId: item.objectId, quantity: item.quantity || menuItemMaxCount(item) }));
}
...
function cancelContainerTransferPanel() {
  ...
  pendingGroundMenuTransferIntent = null;
  if (!isGroundPickup || (currentMenu?.awaitingSelection && !groundReleaseAlreadyRequested)) sendPlayableText('\u001b');
...
function renderContainerItemRow(item, side) {
  ...
  const selectorLabel = itemHasNetHackSelector(item) ? `selector ${key}` : (displayKey ? `stable item key ${displayKey}` : (isGroundPickup ? 'ground row' : 'direct item'));
  const keycap = itemHasNetHackSelector(item) ? key : (displayKey || (isGroundPickup ? 'ground' : '—'));
  button.setAttribute('aria-label', `${sourceLabel} ${cleanName}; ${draggable ? `drag to ${targetLabel} pane; ` : ''}${isGroundPickup && side === 'left' ? 'actions available with right click or Shift+F10; ' : ''}${selectorLabel}`);
  ...
  button.addEventListener('dragstart', (event) => {
    ...
    if (containerTransferState?.active) {
      ...
      containerTransferState.feedback = `Dragging ${cleanName} to ${target}. Release over the highlighted pane to route the NetHack command.`;
...
function reconcileContainerTransferAfterDirectRejection(rawEvent = {}, transferBeforeReject = null) {
  ...
  if (inventoryRows.length || authoritativeInventoryKnown) {
...
async function sendDirectGroundTransfer(item, transfer, itemName, direction) {
  ...
  const command = {
    ...
    targets: { itemId: item.objectId, location: { kind: sourceLocation }, displayName: itemName, coord },
    payload: {
      direction,
      transferId,
      coord,
...
async function sendDirectContainerTransfer(item, transfer, itemName, direction) {
  ...
  const targetLocation = direction === 'inventory-to-container' ? 'container' : 'inventory';
  ...
  const command = {
    ...
    transactionId: transferId,
    expectedRevision: undefined,
    targets: { containerId: identity.objectId, itemId: item.objectId, location: { kind: sourceLocation }, displayName: itemName },
    payload: {
      direction,
      transferId,
      ...
      itemId: item.objectId,
      item: { objectId: item.objectId, displayName: itemName, quantity: Number.isInteger(item.quantity) ? item.quantity : undefined, location: { kind: sourceLocation }, targetLocation: { kind: targetLocation } },
    },
  };
...
function flushPendingContainerTransferSelection(menu) {
  ...
  clearPendingContainerTransferSelection();
  if (!selectorToSend) {
    containerTransferState.feedback = `${pending.action === 'out' ? 'Take-out' : 'Put-in'} menu opened, but selector ${pending.selector} is no longer available (${resolved.reason}). Refreshing the panel.`;
    containerTransferState.reopenPending = true;
    applyTransferChoreographyPatch({ reopenPending: true, refreshIntent: { kind: 'container-direct-refresh-after-missing-selector', side: pending.sourceSide, transferId: pending.transferId || '', command: '', delayMs: 0, reason: containerTransferState.feedback } }, 'container pending selector missing');
    renderContainerTransferPanel();
    sendPlayableText('\u001b');
    return true;
  }
  containerTransferState.feedback = `${pending.action === 'out' ? 'Taking out' : 'Putting in'} ${pending.itemName || `selector ${pending.selector}`}; refreshing both panes after NetHack confirms the transfer.`;
  containerTransferState.reopenPending = true;
...
function transferContainerItem(sourceSide, selector) {
  ...
  const itemNameBeforeMove = itemBeforeMove ? containerTransferDisplayName(itemBeforeMove, sourceSide) : `selector ${selector}`;
  ...
  const commandSelector = activeMenuResolution?.selector || selector;
  if ((activeTakeOut || activePutIn) && !activeMenuResolution?.selector) {
    containerTransferState.feedback = `${takingOut ? 'Take-out' : 'Put-in'} menu is open, but ${itemNameBeforeMove} no longer has an unambiguous current selector (${activeMenuResolution?.reason || 'selector unavailable'}). Refreshing the panel.`;
    containerTransferState.reopenPending = true;
    applyTransferChoreographyPatch({ reopenPending: true, refreshIntent: { kind: 'container-direct-refresh-after-unresolved-active-selector', side: sourceSide, command: '', delayMs: 0, reason: containerTransferState.feedback } }, 'container active menu selector unresolved');
    renderContainerTransferPanel();
    sendPlayableText('\u001b');
    return;
  }
  ...
  if ((takingOut && canUseDirectContainerToInventoryTransfer(itemBeforeMove)) || (!takingOut && canUseDirectInventoryToContainerTransfer(itemBeforeMove))) {
    clearPendingContainerTransferSelection();
  ...
  reconcileContainerTransferAfterDirectRejection({ transferId: transfer?.transferId || '', sessionId: transfer?.sessionId || containerTransferState?.transferSessionId || '', reason }, transfer);
}
  ...
  if (containerTransferState.sessionKind === 'ground-pickup') {
    if (side === 'right') {
      ...
      if (rows.length) {
        ...
        containerTransferState.feedback = 'Ground items refreshed from the latest public ground snapshot; no comma refresh was sent.';
      } else {
        containerTransferState.feedback = 'No public ground snapshot is available to refresh; no comma fallback was sent.';
      }
      renderContainerTransferPanel();
    }
    return;
  }
  containerTransferState.feedback = side === 'left' ? 'Refreshing container contents through a direct NetHack snapshot…' : 'Refreshing inventory candidates from public inventory state…';
  renderContainerTransferPanel();
...
function renderContainerTransferPanel() {
  ...
  title.innerHTML = `<div><strong>${escapeHtml(titleText)}</strong><span>${escapeHtml(displayFeedback)}</span></div>`;
...
function renderStructuredMenuOption(kind, item, key, tile, assetId) {
  ...
  if (kind === 'spell') {
    ...
    if (/skill|enhance|advance/i.test(prompt)) {
      const skill = parseSkillMenuText(item.text);
      const badges = `${skill.rank ? `<span class="item-badge skill-rank">${escapeHtml(skill.rank)}</span>` : ''}${skill.cost ? `<span class="menu-meta">cost ${escapeHtml(skill.cost)}</span>` : ''}${skill.canAdvance ? '<span class="item-badge advance-badge">can advance</span>' : ''}`;
      return `<span class="selector-keycap" aria-hidden="true">${key}</span><span class="row-action-pill">Advance</span><span class="menu-item-main"><span class="menu-item-name">${escapeHtml(skill.name)}</span><span class="menu-badges">${badges}</span></span>`;
    }
    const spell = parseSpellMenuText(item.text);
    const badges = `${spell.level ? `<span class="item-badge spell-level">Lvl ${escapeHtml(spell.level)}</span>` : ''}${spell.power ? `<span class="menu-meta">Pw ${escapeHtml(spell.power)}</span>` : ''}${spell.fail ? `<span class="menu-meta ${parseInt(spell.fail, 10) >= 50 ? 'danger-meta' : ''}">Fail ${escapeHtml(spell.fail)}</span>` : ''}`;
    return `<span class="selector-keycap" aria-hidden="true">${key}</span><span class="row-action-pill">Cast</span><span class="menu-item-main"><span class="menu-item-name">${escapeHtml(spell.name)}</span><span class="menu-badges">${badges}</span></span>`;
  }
  if (kind === 'options') {
    ...
    showInteractionDialog({
      ...
      options: selectable.map((item) => {
        ...
        return {
          ...
          ariaLabel: `${rowAction ? `${rowAction.label}${layeredActionHint}${rowAction.enabled === false ? '; blocked by visible equipment state' : '; draggable to compatible equipment slots'}` : 'Inventory'} ${menuItemName(item.text)}${menuItemState(item.text) ? `, ${menuItemState(item.text)}` : ''}; Actions menu available with right click or Shift+F10; NetHack selector ${key}`,
    ...
    showInteractionDialog({
      ...
      options: selectable.map((item) => {
        ...
        return {
          ...
          className: 'inventory-row rpg-inventory-row',
          filterText: `${menuItemClass(item)} ${menuItemSemanticFilterText(item)} ${menuItemState(item.text)}`,
          ariaLabel: `${menuItemName(item.text)}${menuItemState(item.text) ? `, ${menuItemState(item.text)}` : ''}; NetHack selector ${key}`,
  ...
  if (currentMenu.awaitingSelection) {
    ...
    if (!hasSelection) {
      ...
      showInteractionDialog({
        ...
        cancelText: 'Close / Esc',
        panelControls: readOnlyMenuPanelControls(currentMenu),
        options: [{
          key: ' ',
    ...
    showInteractionDialog({
      ...
      cancelText: hasSelection ? 'Cancel / Esc' : 'Close',
      ...
      options: selectable.map((item) => {
        ...
        return {
          ...
          ariaLabel: `${multi ? 'Toggle' : 'Choose'} ${menuItemName(item.text)}${menuItemState(item.text) ? `, ${menuItemState(item.text)}` : ''}; ${groundPickupMenu ? 'Ground item actions available with right click or Shift+F10; ' : ''}NetHack selector ${key}${!shopPaymentMenu && maxCount > 1 ? `; quantity 1 to ${maxCount}` : ''}`,
          html: kind === 'inventory'
...
function chooseMapTargetCell(cellEl) {
  ...
  const x = cellEl.dataset.mapX;
  const y = cellEl.dataset.mapY;
  appendMessage(adjacentDirection ? `GUI target: adjacent ${adjacentDirection.toUpperCase()}.` : `GUI target: map ${x},${y}.`);
  activePrompt = null;
  activeMapTargetSelection = null;
  setMapTargetMode(null);
  hideDirectionHelper();
...
function applyGameViewEffects(effects) {
  for (const item of effects || []) {
    ...
    if (item.type === 'ground-pile-snapshot') {
      ...
      if (item.type === 'transfer-transaction-rejected' || item.type === 'transfer-transaction-followup-rejected') diagnosticEvent('transaction', 'transfer.rejected', item, { transactionId: item.transfer?.transferId || item.rejection?.transferId || '', requestId: item.transfer?.expectedRequestId || '' });
    }
      ...
      setStatus(`command transaction accepted: ${item.transaction?.semanticAction || 'command'}`);
    }
      ...
      if (paymentStatusActive) setStatus(shopPaymentUiStatus.text);
      else setStatus(item.result?.status === 'failure' ? `command failed: ${item.result?.reason || 'see messages'}` : `command completed: ${item.transaction?.semanticAction || 'command'}`);
    }
    else if (item.type === 'command-transaction-completion-rejected') {
      if (!/already completed/i.test(String(item.reason || ''))) setStatus('Ready.');
    }
...
async function runVersion() {
  ...
  setStatus(result.ok ? 'version check passed' : 'version check failed');
}
...
netHackAPI.onExit((exit) => {
  ...
  if (gameOverState?.active) showGameOverModal();
  else setStatus(exit.expected ? 'game stopped' : 'game exited');
});

netHackAPI.onState((state) => {
  setRunningState(state);
  if (state.running) setStatus(`${state.mode || 'game'} running (pid ${state.pid})`);
});
...
function handleShimEvent(event) {
  const appEvent = sharedModules.shimProtocol?.normalizeRawShimEvent ? sharedModules.shimProtocol.normalizeRawShimEvent(event?.event || event?.raw || event) : { event, name: event?.name };
  const rawEvent = appEvent.event || appEvent.raw || event;
  shimEventCount += 1;
...
function selectedCharacter() {
  syncCharacterSelects();
  return {
    name: (document.getElementById('player-name').value || 'Electron').replace(/[^A-Za-z0-9_]/g, '').slice(0, 24) || 'Electron',
...
function recordCheckpoint(name = '') {
  ...
  activeRecording.events.push({ type: 'checkpoint', t: Math.round(performance.now()), name: checkpointName, state: automationState() });
...
async function saveActiveRecording(reason = 'manual') {
  ...
  activeRecording.endedAt = new Date().toISOString();
  activeRecording.endReason = reason;
...
async function showStartupChoiceIfAppropriate({ force = false } = {}) {
  ...
  if (!startupChoiceDialog.open) startupChoiceDialog.showModal();
...
function selectorForInventoryItem(item = {}) {
  ...
  return String(item.key || item.selector || '').slice(0, 1);
}
...
function currentActionExpectedRevision() {
  const revision = {};
  const inventory = Math.max(publicInventorySnapshot?.revision || 0, liveInventoryRevision || 0);
  const equipment = publicEquipmentSnapshot?.revision || 0;
...
function buildActionExecuteContext() {
  return {
    ...
    inventoryRevision: publicInventorySnapshot?.revision || liveInventoryRevision || 0,
    equipmentRevision: publicEquipmentSnapshot?.revision || 0,
...
function sendGroundContainerExtendedAction(commandKeys, actionId, defaultLabel, action = {}, item = {}) {
  ...
  return sendSemanticActionCommand(commandKeys, { id: actionId, label: action.label || defaultLabel }, item || {}, {
    ...
    target: { location: { kind: 'ground' }, ...(Number.isInteger(item?.objectId) && item.objectId >= 0 ? { objectId: item.objectId } : {}), ...(targetText ? { displayName: targetText } : {}) },
...
function directContainerIdentityForRefresh() {
  const identity = containerSnapshotIdentity();
  if (Number.isInteger(identity.objectId) && identity.objectId > 0) return { objectId: identity.objectId, text: identity.displayName || containerTransferState?.prompt || 'container' };
  const pending = pendingDirectContainerOpen || {};
  if (Number.isInteger(pending.containerId) && pending.containerId > 0) return { objectId: pending.containerId, text: pending.displayName || 'container' };
...
async function requestDirectContainerSnapshotRefresh(reason = 'manual-refresh') {
  ...
  const command = {
    ...
    transactionId: sessionId,
    targets: { containerId: item.objectId, location: { kind: 'ground' }, displayName: cleanEquipmentText(item.text || item.displayName || 'container') },
  ...
  if (!ack?.ok && containerTransferState?.active) {
    clearDirectContainerSnapshotTimeout();
    containerTransferState.feedback = `Direct container refresh rejected: ${ack?.reason || 'bridge did not accept the snapshot command'}.`;
    renderContainerTransferPanel();
  }
  return ack;
}
...
async function sendDirectContainerSnapshotCommand(action = {}, item = {}) {
  ...
  if (!ack?.ok && containerTransferState?.active) {
    clearDirectContainerSnapshotTimeout();
    containerTransferState.feedback = `Direct container open rejected: ${ack?.reason || 'bridge did not accept the snapshot command'}.`;
    pendingDirectContainerOpen = null;
    renderContainerTransferPanel();
  }
  return ack;
...
async function sendDirectEquipmentChangeCommand(command, action = {}, item = {}, route = {}, options = {}) {
  ...
  const sent = await Promise.resolve(netHackAPI.uiCommand?.(plan.command)).catch((error) => ({ ok: false, reason: String(error?.message || error) }));
...
async function sendSemanticActionCommand(keys, action = {}, item = {}, route = {}, options = {}) {
  ...
  if (v2Plan?.ok) {
    ...
    const sent = await Promise.resolve(netHackAPI.uiCommand?.(v2Plan.command)).catch((error) => ({ ok: false, reason: String(error?.message || error) }));
...
function sendRepeatedCommand(commandKey) {
  ...
  appendMessage(`GUI repeat: ${count} × ${commandKey === '.' ? 'wait/rest' : commandKey === 's' ? 'search' : describePlayableKey(commandKey)}.`);
  gameGrid.focus({ preventScroll: true });
}

function movementPrefixForMode(mode = movementMode) {
  if (mode === 'run') return 'g';
...
function sendMovementCommand(directionKey) {
  ...
  repeatCountInput.value = String(count);
  sendPlayableText(`${countPrefix}${prefix}${directionKey}`);
  appendMessage(`GUI movement: ${count > 1 ? `${count} × ` : ''}${movementMode} ${directionKey.toUpperCase()}.`);
  gameGrid.focus({ preventScroll: true });
}
...
function sendExtendedCommandButton(button) {
  ...
  appendMessage(`GUI # command: ${button.textContent.trim()} (#${command}).`);
  gameGrid.focus({ preventScroll: true });
}
...
function refreshEquipmentInventoryAfterCompatAction(feedback = '', attempt = 0) {
  ...
  equipmentRefreshTimer = window.setTimeout(() => {
    ...
    sendPlayableKey('i');
    appendMessage('GUI equipment refresh: requested updated inventory after semantic equipment action.');
  }, attempt ? 250 : 260);
}
...
function sendCommandButton(button) {
  ...
  if ('aezqrtwWTRPxQZ'.includes(key)) appendMessage(`GUI action: ${button.textContent.trim()}${'aeqrztwWTPRQZ'.includes(key) ? '; choose from the visible picker if NetHack asks.' : '.'}`);
  if ('SQ'.includes(key)) appendMessage(`GUI lifecycle action: ${button.textContent.trim()}; confirm using the visible safe/danger buttons.`);
  gameGrid.focus({ preventScroll: true });
}

function openActionDialog() {
  if (hasActiveUiInputOwner() && !actionDialog.open) return;
  if (!actionDialog.open) actionDialog.showModal();
  itemActions?.querySelector('button')?.focus({ preventScroll: true });
}

function closeActionDialog({ refocusGame = true } = {}) {
...
openActionsButton?.addEventListener('click', openActionDialog);
actionDialogClose?.addEventListener('click', () => closeActionDialog());
actionDialog?.addEventListener('cancel', () => setTimeout(() => gameGrid.focus({ preventScroll: true }), 0));
...
function topmostEscapeLayer() {
  const openDialogLayers = modalOverlayDialogs
