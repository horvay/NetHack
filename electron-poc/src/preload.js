...
const version = 'nethack-preload-contract/v1';
...
contextBridge.exposeInMainWorld('netHackPOC', Object.freeze({
  version,
  info: () => ipcRenderer.invoke('nethack:info'),
  runVersion: () => ipcRenderer.invoke('nethack:runVersion'),
  startGame: (size) => ipcRenderer.invoke('nethack:startGame', validStartSize(size)),
  startShimBridge: (options) => ipcRenderer.invoke('nethack:startShimBridge', validShimOptions(options)),
  startupRecoveryState: () => ipcRenderer.invoke('nethack:startupRecoveryState'),
  ...
  ...(process.env.NH_ELECTRON_TEST_FIXTURES === '1' && process.env.NH_TEST_CAPTURE_DIR ? {
    ...
    captureTestScreenshot: (captureId) => ipcRenderer.invoke('nethack:testCapturePage', String(captureId || '').slice(0, 160)),
  } : {}),
  input: (data) => ipcRenderer.send('nethack:input', String(data || '')),
  shimKey: (key) => ipcRenderer.send('nethack:shimKey', validKey(key)),
  shimInput: (payload) => ipcRenderer.send('nethack:shimInput', validShimInput(payload)),
  uiCommand: (command) => ipcRenderer.invoke('nethack:uiCommand', validUiCommand(command)),
  onData: (callback) => subscribe('nethack:data', callback),
  onShimEvent: (callback) => subscribe('nethack:shimEvent', callback),
  onExit: (callback) => subscribe('nethack:exit', callback),
  onState: (callback) => subscribe('nethack:state', callback),
}));
