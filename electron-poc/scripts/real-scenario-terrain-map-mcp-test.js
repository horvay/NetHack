...
async function main() {
  ...
  try {
    ...
    const summary = [`# Scenario loader terrain/map real Electron smoke`, '', 'PASS', '', `Scenario: ${scenarioId}`, `Map screenshot: ${mapShot}`, `Specific trap tooltip: ${trapTooltipShot}`, '', 'Verified scenario public facts through visible UI/map cells:', '- wall cell', '- open door cell', '- pit trap cell and matching pit asset/tooltip', '- water cell', '- lava cell', '- up-stairs cell', '- ground apple object cell', '', 'Map cell evidence:', '```json', evidenceText, '```', '', 'Note: this proof verifies the public map glyph/semantic/aria cells emitted by the real shim startup for the terrain fixture; it does not attempt stair traversal or swimming/lava movement.', ''].join('\n');
