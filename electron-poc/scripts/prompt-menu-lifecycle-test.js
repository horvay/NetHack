...
const requestId = view.state.currentMenu.requestId;
const transactionId = view.state.currentMenu.transactionId;
const lifecycleRevision = view.state.currentMenu.lifecycleRevision;
...
assert.equal(view.state.currentMenu.requestId, requestId, 'request id is stable across start/add/end/select');
assert.equal(view.state.currentMenu.transactionId, transactionId, 'transaction id is stable across start/add/end/select');
assert.equal(view.state.currentMenu.lifecycleRevision, lifecycleRevision, 'lifecycle revision is stable across menu lifecycle');
assert.equal(view.state.activePrompt.requestId, requestId, 'menu selection prompt shares menu request id');
assert.equal(view.state.activePrompt.transactionId, transactionId, 'menu selection prompt shares menu transaction id');
assert.equal(view.state.activePrompt.kind, 'read-only menu', 'read-only information menu remains a legitimate prompt owner');
...
assert(!view.state.currentMenu.items.some((item) => /stale item/.test(item.text)), 'rejected stale add_menu does not mutate current menu');
...
assert(view.state.currentMenu, 'wrong-window untagged menu answer does not close active menu');
...
assert(view.state.currentMenu, 'wrong-window stale menu answer does not close active menu');
...
assert(view.state.currentMenu, 'untagged menu answer does not close active menu');
...
assert(view.state.currentMenu, 'stale menu answer does not close active menu');
process(view, { name: 'bridge_menu_answer', window: 40, requestId, return: 0 });
assert.equal(view.state.currentMenu, null, 'matching menu answer closes active menu');
...
assert.equal(promptView.state.activePrompt.requestId, 'req-current', 'prompt request id preserved');
assert.equal(promptView.state.activePrompt.transactionId, 'v1-command-1--', 'prompt inherits command transaction id');
...
assert.equal(promptView.state.activePrompt.query, 'Really quit? [yn]', 'rejected stale prompt cannot steal active prompt ownership');
...
assert.equal(promptView.state.activePrompt.requestId, 'req-current', 'untagged prompt answer does not close current prompt');
...
assert.equal(promptView.state.activePrompt.requestId, 'req-current', 'stale prompt answer does not close current prompt');
process(promptView, { name: 'bridge_prompt_answer', requestId: 'req-current', keycode: 'n'.charCodeAt(0) });
assert.equal(promptView.state.activePrompt, null, 'matching prompt answer closes current prompt');
...
assert.equal(autoView.state.activePrompt.requestId, 'visible-prompt', 'auto-answered GUI ring-finger prompt does not steal visible prompt ownership');
...
assert.equal(autoView.state.activePrompt.requestId, 'visible-prompt', 'auto-answered GUI ring-finger answer does not clear unrelated prompt');
process(autoView, { name: 'bridge_prompt_answer', requestId: 'visible-prompt', keycode: 'n'.charCodeAt(0) });
assert.equal(autoView.state.activePrompt, null, 'matching visible prompt answer clears prompt');
...
assert.equal(autoView.state.activePrompt, null, 'auto-answered GUI ring-finger prompt remains hidden when no prompt is active');
const manualRing = process(autoView, { name: 'shim_yn_function', query: 'Which ring-finger, Right or Left? [rl]', choices: 'rl' });
assert.equal(autoView.state.activePrompt.promptPurpose, 'prompt.equipmentRingFinger', 'manual ring-finger prompt remains visible and classified');
assert.equal(autoView.state.activePrompt.owner.kind, 'equipment', 'manual ring-finger prompt has equipment owner');
