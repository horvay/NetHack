...
assert('unidentified crude dagger uses crude/orcish-dagger public-appearance art, not crossed weapon class fallback', crudeDagger.assetId === 'orcish-dagger' && crudeDagger.classes.includes('terrain-floor') && /orcish-dagger\.png/.test(crudeDagger.backgroundImage || crudeDagger.tileImage || '') && /Crude Dagger/i.test(crudeDagger.ariaLabel || '') && crudeDaggerTooltip.title === 'Crude Dagger' && crudeDaggerTooltip.assetId === 'orcish-dagger' && /Objects and inventory items · glyph 3484 · map 23,13/.test(crudeDaggerTooltip.description || '') && !/weapon-class-icon|Full source objects/i.test(`${crudeDagger.assetId} ${crudeDagger.backgroundImage || ''} ${crudeDagger.tileImage || ''} ${crudeDagger.ariaLabel || ''} ${crudeDaggerTooltip.description}`) && !/orcish dagger/i.test(`${crudeDagger.ariaLabel || ''} ${crudeDaggerTooltip.title || ''} ${crudeDaggerTooltip.description || ''}`), JSON.stringify({ crudeDagger, crudeDaggerTooltip }));
...
assert('thin spellbook tooltip uses public appearance title and spellbook art', thinSpellbookTooltip.title === 'Thin Spellbook' && thinSpellbookTooltip.assetId === 'spellbook-class-icon' && /Objects and inventory/i.test(thinSpellbookTooltip.description) && !/jumping|closed door/i.test(`${thinSpellbookTooltip.title} ${thinSpellbookTooltip.description}`), JSON.stringify(thinSpellbookTooltip));
...
assert('unknown wand tooltip uses public appearance title and wand art', unknownWandTooltip.title === 'Long Wand' && unknownWandTooltip.assetId === 'wand-class-icon' && /Objects and inventory/i.test(unknownWandTooltip.description) && !/death|open door/i.test(`${unknownWandTooltip.title} ${unknownWandTooltip.description}`), JSON.stringify(unknownWandTooltip));
...
assert('actor/object tooltip keeps both actor and object information', /Hero/i.test(heroOverBoxTooltip.title) && /Also here: Large Box/.test(heroOverBoxTooltip.description), JSON.stringify(heroOverBoxTooltip));
...
assert('monster/object tooltip keeps both monster and object information', /Jackal/i.test(jackalOverChestTooltip.title) && /Also here: Chest/.test(jackalOverChestTooltip.description), JSON.stringify(jackalOverChestTooltip));
