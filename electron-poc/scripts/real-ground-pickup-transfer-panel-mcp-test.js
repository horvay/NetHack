...
async function start(cdp) {
  ...
  await waitFor(async () => { const s = await state(cdp); const failure = s.shimEvents.find((event) => event.name === 'bridge_test_scenario_failed'); if (failure) throw new Error(JSON.stringify(failure)); return s.running && s.shimEvents.some((event) => event.name === 'shim_print_glyph' || event.name === 'shim_status_update' || event.name === 'shim_putstr') ? s : null; }, 25000);
