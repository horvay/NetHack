...
async function main() {
  ...
  try {
    await start(cdp);
    const loaded = await waitFor(async () => { const s = await state(cdp); const failure = s.shimEvents.find((event) => event.name === 'bridge_test_scenario_failed'); if (failure) throw new Error(JSON.stringify(failure)); return s.shimEvents.some((event) => event.name === 'bridge_test_scenario_loaded') ? s : null; }, 10000).catch(async (error) => { const debug=await state(cdp).catch(()=>({})); fs.writeFileSync(path.join(outDir,'scenario-load-timeout-debug.json'),JSON.stringify(debug,null,2)); await shot(cdp,'scenario-load-timeout.png').catch(()=>undefined); throw error; });
    assert('scenario loaded event visible', loaded.shimEvents.some((event) => event.name === 'bridge_test_scenario_loaded'), JSON.stringify(loaded.shimEvents.slice(-20)));
    const initialContextState = await waitFor(async () => {
      ...
      return s.actions?.buttons?.some((b) => b.id === 'pickup') && s.shimEvents.some((event) => event.name === 'shim_ground_pile_snapshot') && authoritativePile ? { ...s, authoritativePile } : null;
    }, 10000);
