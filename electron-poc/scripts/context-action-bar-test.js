...
const checks = [
  ...
  ['base labels are player-facing and not raw command letters', /label: 'Search'/.test(interaction) && /label: 'Wait'/.test(interaction) && /label: 'Inspect \/ look'/.test(interaction) && !/>o<|>,<|>>&lt;/.test(html.match(/id="context-action-bar"[\s\S]*?<div id="play-area"/)?.[0] || '')],
