...
assert.equal(view.snapshot().currentMenu.menuPurpose, 'inventory.overview', 'game-view-state stores current menu purpose');
assert.equal(view.snapshot().currentMenu.requestId, 'req-state-menu', 'game-view-state stores current menu request id');
assert.equal(view.snapshot().currentMenu.owner.kind, 'inventory', 'game-view-state stores current menu owner');
assert.equal(view.snapshot().currentMenu.selectionMode, 'none', 'game-view-state stores selection mode');
assert.equal(view.snapshot().currentMenu.lifecycle, 'ready', 'game-view-state stores lifecycle facts');
...
assert.equal(selectableView.snapshot().currentMenu.selectionMode, 'many', 'select_menu how=2 updates selection mode');
assert.equal(selectableView.snapshot().currentMenu.lifecycle, 'selecting', 'selectable menu lifecycle becomes selecting');
const selectableRequestId = selectableView.snapshot().currentMenu.requestId;
selectableView.process({ name: 'bridge_menu_answer', window: 41, requestId: selectableRequestId, return: 1, selectors: 'a' });
assert.equal(selectableView.snapshot().currentMenu, null, 'bridge_menu_answer closes stale current menu in shared state');
assert.equal(selectableView.snapshot().menusByWindow.has(41), false, 'bridge_menu_answer clears answered menu window history');
assert.equal(selectableView.snapshot().pendingMenuSelections.has(41), false, 'bridge_menu_answer clears pending menu selection state');
selectableView.process({ name: 'shim_yn_function', query: 'Really proceed? [yn]', choices: 'yn' });
assert.equal(selectableView.snapshot().currentMenu, null, 'follow-up prompt does not coexist with stale current menu');
assert.equal(selectableView.snapshot().activePrompt.promptPurpose, 'prompt.question', 'follow-up prompt remains active with metadata');
...
const cancelRequestId = cancelView.snapshot().currentMenu.requestId;
cancelView.process({ name: 'bridge_menu_answer', window: 44, requestId: cancelRequestId, return: 0, selectors: '' });
assert.equal(cancelView.snapshot().currentMenu, null, 'canceled selectable bridge_menu_answer clears current menu');
assert.equal(cancelView.snapshot().menusByWindow.has(44), false, 'canceled selectable bridge_menu_answer clears menu window history');
cancelView.process({ name: 'shim_yn_function', query: 'Really quit? [yn]', choices: 'yn' });
assert.equal(cancelView.snapshot().currentMenu, null, 'follow-up prompt after canceled selectable menu has no stale current menu');
assert.equal(cancelView.snapshot().activePrompt.promptPurpose, 'prompt.question', 'follow-up prompt after cancel remains active with metadata');
...
assert.equal(promptlessView.snapshot().currentMenu.menuPurpose, 'inventory.overview', 'promptless inventory uses last inventory command context in shared state');
...
assert.equal(view.snapshot().activePrompt.promptPurpose, 'prompt.itemAction', 'active prompt stores prompt purpose');
assert.equal(view.snapshot().activePrompt.owner.kind, 'action', 'active prompt stores owner');
assert(view.snapshot().activePrompt.requestId, 'active prompt stores request id');
