...
(async () => {
  ...
  try {
    ...
    results.locked = await evalExpr(cdp, `(() => { const t=window.__nethackPromptTest; t.reset(); t.setRunning(true); t.setAuthoritativeInventoryForTest([{ selector:97, objectId:2001, displayName:'key', semanticAppearance:'key', semanticKnown:false, quantity:1, publicClass:'tool', actionAffordances:['apply'] }], 5001); document.dispatchEvent(new KeyboardEvent('keydown',{key:'l',bubbles:true})); t.clearSentInputs(); t.event({ name: 'shim_putstr', text: 'This door is locked.' }); return { dialog: t.dialog(), context: t.context() }; })()`);
    results.screenshots.locked = await shot(cdp, '01-locked-door-kick-context.png');
    await evalExpr(cdp, `(() => { const button=Array.from(document.querySelectorAll('#interaction-options .choice-button')).find((candidate)=>/Unlock with key/i.test(candidate.innerText)); button?.click(); return Boolean(button); })()`);
    await delay(180);
    results.freshKeyDispatch = await evalExpr(cdp, `window.__nethackPromptTest.sentInputs().join('')`);
    ...
    results.checks.lockedDoorKickContextAppears = results.locked.dialog.interactionOpen && /Locked door actions/i.test(results.locked.dialog.title || '') && results.locked.dialog.options.some((o) => /Kick door/i.test(o.text || '')) && results.locked.dialog.options.some((o) => /Unlock with key/i.test(o.text || ''));
    results.checks.freshKeyDispatchAppliesSelectedKeyTowardLockedDoor = results.freshKeyDispatch === 'aal';
